...
class CsvListener(QWidget):
    def __init__(self, csv_file, audio_path=None, do_exclude=False, do_comment=False, 
                 delimiter=',', fn_file_name='File Name', fn_exclude='Exclude', 
    ...
    def read_csv(self):
        with open(self.csv_file, encoding='utf8') as inf:
            if self.delimiter == 'tab':
                reader = csv.DictReader(inf, delimiter='\t')
            else:
                reader = csv.DictReader(inf, delimiter=self.delimiter)
    ...
    def save_file(self):
        with open(self.csv_out, 'w') as outf:
            if self.delimiter == 'tab':
                ...
                writer = csv.DictWriter(outf, delimiter=self.delimiter, fieldnames=self.csv_header, 
                                        extrasaction='ignore', lineterminator='\n')
            writer.writeheader()
            if self.do_exclude:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--delimiter', help="Delimiter to split input file fields. For tabs, pass the word 'tab', anything else just pass the literal character. Default comma", default=',')
